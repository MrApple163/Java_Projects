...
{	
	...
	public Frame(String title)
	{
		...
		res.addActionListener(new ActionListener() // кнопка очистки поля и сброса всех счетчиков
			...
			{
				kx = 0; ko = 0; k = 0; // счетчики победы Х, победы О, хода
		...
		ai.addActionListener(new ActionListener() // кнопка вкл. выкл. ИИ
				{
			public void actionPerformed(ActionEvent e)
			{
		...
		btn[0].addActionListener(new ActionListener() // кнопки игрового поля
				{
			public void actionPerformed(ActionEvent e)
			{
				if (b[0] == false)
				{
					...
					// прибавляется счетчик хода(k) и изменяется значение boolean отвечающей за наличие текста на кнопке
					// если k нечетный то на кнопку выводится Х, иначе О
				}
			}
				});
		...
		win = new Timer(250, new ActionListener()
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					JOptionPane.showMessageDialog(null, str[0] + " win"); // проверяются все 8 возможных условий победы
					if (str[0] == "X") kx ++;
					else ko ++;
					lab[1].setText(kx + ""); lab[3].setText(ko + "");
					for (int i = 0; i < 9; i ++)
